...
router.get('/quiz_attempts/:attempt_id/score', (req, res) => {
  ...
  // Query to get the score, deduction, and max possible score for a specific quiz attempt
  ...
  db.query(selectScoreQuery, [attempt_id], (err, results) => {
    ...
    const deduction_percentage = results[0].deduction_percentage;
    const max_score = results[0].max_score || 0;  

    return res.status(200).json({ score, deduction_percentage, max_score });
  });
});
